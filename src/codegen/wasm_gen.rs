...
pub fn generate_wasm_output(
    affine_module: &AffineModule,
...
fn generate_build_config(config: &WasmConfig, module_name: &str) -> String {
    ...
    writeln!(out, "# WASM build configuration generated by affinescriptiser").unwrap();
    ...
    for line in affine_module.source.lines() {
        ...
        if trimmed.starts_with("fn safe_create_") || trimmed.starts_with("fn safe_destroy_") {
            ...
            if let Some(name) = extract_function_name(trimmed) {
                ...
                .unwrap();
                writeln!(out, "  {}.{}(/* forwarded args */)", affine_module.module_name, name).unwrap();
                writeln!(out, "}}").unwrap();
                writeln!(out).unwrap();
            }
        }
...
mod tests {
    use super::*;
    use crate::abi::{AffinityKind, AffineResource};
    ...
    fn test_check_size_limit_exceeded() {
        ...
        assert!(violation.unwrap().violation.to_string().contains("SIZE-EXCEEDED"));
    }
    ...
    fn test_extract_function_name() {
        assert_eq!(
            extract_function_name("fn safe_create_gpu_buffer(params: AllocParams) -> GpuBuffer<Affine> {"),
